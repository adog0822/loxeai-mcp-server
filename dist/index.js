...
async function runVerify(argv) {
    ...
    process.stderr.write(`files fingerprinted: ${fp.fileCount}${fp.truncated ? " (TRUNCATED -- covers a subset)" : ""}\n`);
    if (!expected)
        return 0;
    ...
    process.stderr.write("MISMATCH: this tree is NOT the one that produced that report. The files changed, " +
...
async function main() {
    ...
    detectScanners()
        .then((found) => {
        if (found.length === 0) {
            console.error(`[${SERVER_NAME}] no IaC scanner found on PATH. Install checkov or trivy; ` +
                "scan_iac will return install instructions until then.");
        }
        else {
            console.error(`[${SERVER_NAME}] scanners available: ${found.map((s) => `${s.name} (${s.version})`).join(", ")}`);
        }
    })
        .catch((error) => {

...
export function remediationGroupKey(item) {
...
export function getRemediationMeta(input) {
    const title = normalize(input.title);
    const exact = EXACT_TITLE_META[title];
    if (exact)
        return exact;
    if (title.includes("access analyzer"))
        return accountWide;
    if (title.includes("root account") || title.includes("root user"))
        return accountWide;
    if (title.includes("organizations") || title.includes("organization trail"))
        return accountWide;
    if (title.includes("cloudtrail") && !title.includes("trail log file validation"))
        return accountWide;
    if (title.includes("guardduty") || title.includes("security hub") || title.includes("config recorder"))
        return perRegion;
    if (title.includes("aws config") || title.includes("macie") || title.includes("detective"))
        return perRegion;
    if (title.includes("mfa") || title.includes("password") || title.includes("inactive user") || title.includes("access key"))
        return perUser;
    if (title.includes("github repository") || title.includes("repo "))
        return perResource;
    if (title.includes("bucket") || title.includes("security group") || title.includes("rds") || title.includes("ebs"))
        return perResource;
    if (title.includes("lambda") || title.includes("ecr") || title.includes("kms") || title.includes("secret"))
        return perResource;
    return perResource;
}
export function groupRemediationItems(items) {
    ...
    for (const item of items) {
        ...
        if (current) {
            ...
            if ((severityRank[item.severity] ?? 9) < (severityRank[current.severity] ?? 9))
...
//# sourceMappingURL=remediation-meta.js.map
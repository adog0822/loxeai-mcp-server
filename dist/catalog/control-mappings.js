...
import { getControl, IAC_EVIDENCE_CAPABILITY } from "./soc2-controls.js";
const KEYWORD_RULES = [
...
export function loadOverrides(path) {
    ...
    try {
        ...
        if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
            const clean = {};
            for (const [checkId, controlId] of Object.entries(parsed)) {
                ...
                if (!control) {
                    console.error(`[loxeai-mcp] ignoring mapping override ${checkId} -> ${String(controlId)}: not a supported control ID`);
                }
                    ...
                    console.error(`[loxeai-mcp] ignoring mapping override ${checkId} -> ${controlId}: an IaC scan cannot evidence ` +
        ...
        console.error(`[loxeai-mcp] could not read mapping overrides at ${target}: ${error.message}`);
    }
    cachedOverrides = {};
    return cachedOverrides;
}
/** Test seam. */
export function __resetOverridesCache() {
    cachedOverrides = null;
}
// ---------------------------------------------------------------------------
// Mapping
// ---------------------------------------------------------------------------
function unmapped(rationale) {
    return {
        controlId: null,
        controlTitle: null,
        requirement: null,
...
function build(control, mappingSource, confidence, rationale) {
...
export function mapFindingToControl(input) {
    ...
    for (const rule of KEYWORD_RULES) {
        ...
        if (resourceAgrees) {
            return build(control, "resource-attribute", "medium", `${rule.why} Matched on check-name term "${matchedTerm}", corroborated by resource type "${resourceType}".`);
        }
        return build(control, "keyword", "low", `${rule.why} Matched on check-name term "${matchedTerm}" only; resource type "${resourceType ?? "unknown"}" did not corroborate. Verify before relying on this.`);
    }
    // 3. Resource type alone is too weak a signal to assert a control. Say so.
    return unmapped(`No keyword rule matched check name "${checkName ?? ""}". Resource type alone is not a sufficient signal to assert a control mapping. Treat this finding as security-relevant but unmapped.`);
}
/** All controls an IaC scan can evidence at all — primary or partial. */
export function iacAddressableControls() {
 ...
 */
export function mappableControlIds() {

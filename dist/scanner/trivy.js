 ...
 * install.
 */
export function parseTrivyJson(stdout) {
    ...
    const out = { findings: [], parseErrors: [], frameworks: [], suppressions: [], evaluatedResources: 0, evaluatedChecks: 0 };
    ...
    for (const entry of results) {
        ...
        if (misconfSummary) {
            ...
            // evaluated resource.
            if (successes + failures + exceptions > 0)
                out.evaluatedResources += 1;
        }
        ...
        if (!misconfSummary && !Array.isArray(result["Misconfigurations"])) {
            out.parseErrors.push(`${target} produced no misconfiguration results and no summary; Trivy may not have been able to read it`);
        }
        ...
        for (const item of misconfigs) {
            ...
            const lineRange = typeof startLine === "number" && typeof endLine === "number" ? [startLine, endLine] : null;

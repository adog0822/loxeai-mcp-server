...
export async function detectScanners(force = false) {
    if (cache && !force)
        return cache;
    const found = [];
    const checkov = await probe("checkov", ["--version"]);
    if (checkov)
        found.push({ name: "checkov", version: checkov, command: "checkov" });
 ...
 */
export function assertTrustedPath(inputPath) {
    ...
    if (!isAbsolute(inputPath)) {
        return {
            ok: false,
            reason: `Path "${inputPath}" is relative. Supply an absolute path -- run \`pwd\` in the ` +
    ...
    try {
        ...
        return { ok: false, reason: `Path could not be resolved: ${error.message}` };
    }
    if (resolved !== lexical) {
    ...
    for (const denied of DENIED_SEGMENTS) {
        if (haystack.includes(denied.toLowerCase())) {
            return {
                ok: false,
                reason: `Refusing to scan ${resolved}: the path is inside a credential directory ` +
    ...
    if (!inRoot) {
        return {
            ok: false,
            reason: `Refusing to scan ${resolved}: it is outside every trusted root.\n\n` +

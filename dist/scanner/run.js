 ...
 */
import { detectScanners, SCANNER_INSTALL_HELP, assertTrustedPath } from "./detect.js";
...
function pickScanner(available, preferred) {
...
export async function performScan(options) {
    ...
    if (!trust.ok)
        throw new TrustError(trust.reason);
    ...
    if (options.configPath !== undefined) {
        ...
        if (!configTrust.ok) {
            throw new TrustError(`Refusing the supplied configPath. ${configTrust.reason}\n\n` +
    ...
    const frameworks = raw.frameworks.length > 0 ? raw.frameworks : ["unknown"];

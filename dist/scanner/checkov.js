...
function relativePath(input, root) {
    if (!input)
        return "(unknown file)";
    let out = input;
    if (out.startsWith("/")) {
...
function normalizeFramework(checkType) {
...
function toFinding(raw, checkType, root) {
...
function parseReport(report, root, out) {
    const checkType = report["check_type"];
    const framework = normalizeFramework(checkType);
    if (framework !== "unknown" && !out.frameworks.includes(framework))
        out.frameworks.push(framework);
    ...
    if (Array.isArray(skipped)) {
        for (const entry of skipped) {
            ...
            const reason = asString(info?.["suppress_comment"]) ??
    ...
    if (Array.isArray(errors) && errors.length > 0) {
        ...
        for (const entry of errors) {
            ...
            if (text)
                out.parseErrors.push(relativePath(text, root));
        }
    }
}
/**
 ...
 */
function reconcileSuppressions(out) {
    for (let i = out.suppressions.length; i < out.summarySkipped; i += 1) {
        out.suppressions.push({
            ...
            reason: "The scanner reported this suppression in its summary but did not include the detail. " +
                "Search your configuration for `checkov:skip` to identify it.",
        });
    }
}
export function parseCheckovJson(stdout, root) {
    const trimmed = stdout.trim();
    const out = { findings: [], parseErrors: [], frameworks: [], suppressions: [], evaluatedResources: 0, evaluatedChecks: 0, summarySkipped: 0 };
    if (trimmed.length === 0) {

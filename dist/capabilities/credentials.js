...
export function classifyGithubScopes(scopes) {
    if (scopes.length === 0)
        return { capability: "unknown", writeCapableVia: [] };
    const writeCapableVia = scopes.filter((s) => !GITHUB_READ_ONLY_SCOPES.has(s));
    if (writeCapableVia.length === 0)
        return { capability: "read-only", writeCapableVia: [] };
...
async function run(cmd, args, timeoutMs = 15_000) {
    try {
        ...
        const e = error;
        if (e.code === "ENOENT")
            return { ok: false, out: "__NOT_INSTALLED__" };
...
async function probeAws() {
    ...
    if (!ident.ok) {
        return {
            ...base,
            assessment: "The AWS CLI is installed but no valid credentials resolved (expired SSO session, or no profile configured).",
        };
    }
    try {
        const parsed = JSON.parse(ident.out);
        base.authenticated = true;
...
export async function buildCapabilityReport() {
    ...
    return {
        serverGuarantee: "This server performs no write operations of any kind: no filesystem writes, no mutating API calls. Remediation is returned as text for your agent to apply through its own approval step.",
        credentials,

...
export const POLICY_CATALOG: Record<PolicyKind, PolicyMeta> = {
  "information-security": {
...
export type PolicyDraft = {
  ...
  title: string;
  markdown: string;
  supportsCriteria: Array<{ id: string; title: string }>;
  alsoRequires: string[];
...
const HEADER_NOTE = (title: string, criteria: string[]) =>
  [
...
function workforceClause(a?: Answers): string {
  if (!a) return "{{WHO_THIS_APPLIES_TO — employees, contractors, or both}}";
  switch (a.workforce) {
    case "employees":
...
function accessReviewCadence(a?: Answers): string {
...
function buildBody(input: DraftInput): { body: string; grounded: string[]; warnings: string[] } {
  const { kind, answers: a, batch } = input;
  const company = input.companyName ?? "{{COMPANY_NAME}}";
  const grounded: string[] = [];
  const warnings: string[] = [];

  if (a) grounded.push(`Workforce model: ${a.workforce}`);
  if (a) grounded.push(`Production access: ${a.productionAccess}`);
  if (a) grounded.push(`Change process: ${a.changeProcess}`);
  if (batch) {
    ...
    grounded.push(
      `Scan ${batch.batchId}: ${batch.findings.length} finding(s) across ${batch.fileCount} file(s), fingerprint ${batch.fingerprint.slice(0, 12)}`,
    );
    ...
    if (batch.parseErrors.length > 0) {
      warnings.push(
        `The scan this draft references had ${batch.parseErrors.length} parse error(s), so its coverage is incomplete. Do not describe your infrastructure as assessed on the strength of it.`,
      );
    }
    if (batch.suppressions.length > 0) {
      warnings.push(
        `${batch.suppressions.length} check(s) were suppressed in source. If this policy claims a control is enforced, confirm the suppressions do not contradict it.`,
      );
    }
  }
  ...
  switch (kind) {
    case "access-control": {
      ...
      if (a?.productionAccess === "solo") {
        warnings.push(
          "Single-operator production access. CC6.3 cannot be satisfied by separating duties between people — you must name compensating controls explicitly.",
        );
      }
      return {
        body: `## Purpose
...
export function draftPolicy(input: DraftInput): PolicyDraft {

...
export function registerPrompts(server: McpServer): void {
  ...
  server.registerPrompt(
    "remediate_finding",
    {
      ...
      description:
        "Builds a grounded remediation brief for a single finding: the control requirement it " +
    ...
    ({ batchId, findingId }) => {
      ...
      if (!batch) {
        return userMessage(
          `Batch "${batchId}" is not in memory. Run scan_iac first, then retry with the new batchId.`,
        );
      }
      ...
      if (!finding) {
        return userMessage(
          `No finding "${findingId}" in batch ${batchId}. Call list_findings({ batchId: "${batchId}" }) for valid IDs.`,
        );
      }
  ...
  server.registerPrompt(
    "map_repo_to_soc2",
    {
      ...
      description:
        "Produces an honest coverage summary: which SOC 2 criteria this repository's IaC can " +
    ...
    ({ batchId }) => {
      ...
      const processOnly = SOC2_CONTROL_OPTIONS.filter(
        (control) => IAC_EVIDENCE_CAPABILITY[control.id]?.capability === "none",
      );
      ...
      const body = [
        ...
        ...processOnly.map(
          (control) => `  ${control.id} - ${control.title}: ${IAC_EVIDENCE_CAPABILITY[control.id]?.note ?? ""}`,
        ),
        "",

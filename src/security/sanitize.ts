 ...
 */
function isTerminalControl(cp: number): boolean {
  if (cp === 0x09 || cp === 0x0a) return false; // TAB, LF are legitimate
  if (cp <= 0x1f) return true; // C0, including ESC (0x1b) and CR (0x0d)
  if (cp === 0x7f) return true; // DEL
  if (cp >= 0x80 && cp <= 0x9f) return true; // C1, including CSI (0x9b)
  return false;
}

function stripInvisible(input: string): { text: string; removed: number } {
  let removed = 0;
  let out = "";
  for (const char of input) {
    ...
    const hidden =
      isTerminalControl(cp) || INVISIBLE_RANGES.some(([lo, hi]) => cp >= lo && cp <= hi);
    if (hidden) {
      removed += 1;
      continue;
    }
...
export function sanitizeDeep<T>(value: T): { value: T; modified: boolean; stats: { invisibleCharsRemoved: number; injectionPatternsNeutralized: number } } {
  ...
  const sanitized = walk(value) as T;
  return {
    value: sanitized,
 ...
 */
export function wrapUntrusted(content: string, kind: string): string {

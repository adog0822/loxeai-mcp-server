...
export function registerResources(server: McpServer): void {
  ...
  server.registerResource(
    ...
    "loxe://controls/soc2",
    {
      ...
      description:
        "All 33 SOC 2 Common Criteria (Security category) this server catalogues, " +
    ...
    async (uri) =>
      json(uri.href, {
        ...
        notCovered:
          "The optional Availability (A1.x), Confidentiality (C1.x), Processing Integrity (PI1.x) " +
  ...
  server.registerResource(
    "soc2-control",
    new ResourceTemplate("loxe://controls/soc2/{controlId}", {
      list: undefined, // required key, even as undefined
      complete: {
        controlId: (value: string) =>
          SUPPORTED_CONTROL_IDS.filter((id) => id.toLowerCase().startsWith(value.toLowerCase())),
      },
    }),
    {
  ...
  server.registerResource(
    ...
    "loxe://mappings/rules",
    {
      ...
      description:
        "How this server derives SOC 2 control mappings from scanner findings, and how to read " +
    ...
    async (uri) =>
      json(uri.href, {
        ...
        whyNoHardcodedCheckIdTable:
          "Scanner check IDs change between releases and cannot be verified from inside this " +
        ...
        overridesFile:
          "Set LOXE_MAPPING_OVERRIDES to a JSON file of { \"CKV_AWS_19\": \"CC6.7\" }. Validate " +
  ...
  server.registerResource(
    "scan-summary",
    new ResourceTemplate("loxe://scans/{batchId}/summary", {
      list: undefined,
      complete: {
        batchId: (value: string) => listBatchIds().filter((id) => id.startsWith(value)),
      },
    }),
    {
    ...
    async (uri, variables) => {
      ...
      return json(uri.href, {
        ...
        caveat:
          "An IaC scan evidences intended configuration, not deployed state. Do not report a " +
  ...
  server.registerResource(
    ...
    "loxe://workflow/remediation",
    {
      title: "IaC remediation workflow",
      description:
        "The scan -> fix -> rescan loop, its ordering rules, guardrails, and the intended-vs-deployed honesty rule.",

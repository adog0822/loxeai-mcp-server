...
type OscalProp = { name: string; value: string; ns?: string };

const NS = "https://loxe.ai/ns/oscal";

function observationFor(finding: Finding, batch: Batch): Record<string, unknown> {
  return {
    uuid: deterministicUuid(`${batch.fingerprint}:obs:${finding.id}`),
    title: finding.checkName,
    description: `Static analysis of infrastructure-as-code found that ${finding.resource} in ${finding.filePath}${
      finding.lineRange ? ` (lines ${finding.lineRange[0]}-${finding.lineRange[1]})` : ""
    } does not meet the expected configuration for check ${finding.checkId}.`,
    methods: ["TEST"],
...
function findingFor(
  ...
  return {
    ...
    description:
      `An automated infrastructure-as-code scan surfaced ${findings.length} exception(s) that map to ${controlId}. ` +
...
export function buildOscalAssessmentResults(batch: Batch): Record<string, unknown> {
  ...
  return {
    "assessment-results": {
        ...
        roles: [
          {
            ...
            description:
              "Generated by @loxeai/mcp-server running locally. Not an audit, examination, attestation or opinion, and not prepared or reviewed by a licensed CPA firm.",
          },
        ],
        props: [
          ...
          { name: "fingerprint-truncated", value: String(batch.fingerprintTruncated), ns: NS },
          {
            ...
            value:
              "Control IDs reference the AICPA SOC 2 Trust Services Criteria (Common Criteria). No official OSCAL catalog for SOC 2 exists; these identifiers are used descriptively. SOC 2 and Trust Services Criteria are property of the AICPA; this tool is unaffiliated.",
            ns: NS,
          },
          {
            name: "status-semantics",
            value:
              "Only 'not-satisfied' findings are emitted. This tool never asserts 'satisfied': the absence of a scanner finding is not evidence that a criterion is met, and that determination is reserved to a licensed CPA firm.",
            ns: NS,
          },
          {
            name: "evidence-limitation",
            value:
              "Derived from declared infrastructure-as-code. Not evidence of deployed runtime state, and not evidence of operating effectiveness over time.",
            ns: NS,
          },
        ] satisfies OscalProp[],
      },
      ...
      "import-ap": {
        ...
        remarks:
          "No formal OSCAL assessment plan governs this scan. It was produced ad hoc by a local static-analysis tool. This field is present because the OSCAL schema requires it.",
      },
      results: [
        {
          ...
          description:
            `Automated scan of ${batch.fileCount} infrastructure-as-code file(s) using ` +
          ...
          "reviewed-controls": {
            description:
              "Only criteria to which at least one scanner finding mapped are listed. Criteria not listed were either unreachable by an infrastructure scan or produced no findings; neither case implies satisfaction.",
            "control-selections": [

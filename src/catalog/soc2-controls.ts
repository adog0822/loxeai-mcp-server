...
export const SUPPORTED_CONTROL_IDS: string[] = SOC2_CONTROLS.map((c) => c.id);

export function getControl(controlId: string): Soc2Control | undefined {
  return SOC2_CONTROLS.find((control) => control.id === controlId);
}

export function controlsByGroup(): Array<{ group: string; controls: Soc2Control[] }> {
  const order: string[] = [];
  const map = new Map<string, Soc2Control[]>();
...
export function iacPrimaryControls(): Soc2Control[] {
...
export function coverageSummary(): {
  ...
  const iac = { primary: 0, partial: 0, none: 0 } as Record<IacCapability, number>;
...
export const SOC2_CONTROL_OPTIONS: Soc2Control[] = SOC2_CONTROLS;

export type IacEvidenceCapability = IacCapability;

/** @deprecated Read `iac` and `iacNote` off the control directly. */
export const IAC_EVIDENCE_CAPABILITY: Record<string, { capability: IacCapability; note: string }> =
  Object.fromEntries(SOC2_CONTROLS.map((c) => [c.id, { capability: c.iac, note: c.iacNote }]));

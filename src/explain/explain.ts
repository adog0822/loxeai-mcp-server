...
const AUDITOR_FRAMING: Record<string, string> = {
  "CC6.1":
    "CC6.1 is about whether your access architecture is deliberately designed — that protected assets are identified and defended. An auditor reading your Terraform is checking whether the protection was designed in, or bolted on after someone asked.",
  "CC6.3":
    "CC6.3 covers least privilege and segregation of duties. Over-broad permissions in code are one of the easiest things for an auditor to spot and one of the hardest to argue away, because the policy document says exactly what it grants.",
  "CC6.6":
    "CC6.6 covers protection against threats from outside your system boundary. An ingress rule open to the world is the single most legible finding in a SOC 2 examination — it needs no interpretation.",
  "CC6.7":
    "CC6.7 covers protecting information as it moves and restricting where it can go. Unencrypted storage or transport is a direct, documentable gap against this criterion.",
  "CC6.8":
    "CC6.8 covers preventing or detecting unauthorized software. Baseline images and admission controls are part of how entities commonly address it.",
  "CC7.1":
    "CC7.1 covers detecting configuration changes that introduce vulnerabilities. Ironically, running this scanner regularly is itself part of how entities address CC7.1 — but the finding below is a gap in the baseline it is checking against.",
  "CC7.2":
    "CC7.2 covers monitoring for anomalies. Missing logging or alerting configuration means there would be nothing to detect an anomaly with.",
  "CC8.1":
    "CC8.1 covers whether changes are authorized, tested and approved. Because your infrastructure is in version control, the fix for this finding will itself become change-management evidence — the pull request is the artifact.",
  "CC5.2":
    "CC5.2 covers general controls over technology. This finding is one input into whether those controls are actually implemented rather than merely described in a policy.",
  "CC9.1":
    "CC9.1 covers mitigating risks from business disruption. Backup, redundancy and recovery configuration are how entities commonly address it.",
};

const EFFORT_DESCRIPTION: Record<EffortShape, string> = {
...
function effortShapeFrom(finding: Finding): EffortShape {
...
function riskLanguage(finding: Finding): string {
  const subject = finding.resourceType || "this resource";

  if (finding.severity === "UNKNOWN") {
    return (
      `The scanner did not assign a severity to this check, so the risk is not rated. ` +
...
export function explainFinding(finding: Finding, affectedInThisScan: number): FindingExplanation {
  ...
  // ---- what it means ----
  const whatThisMeans =
    `The check "${finding.checkName}" looked at ${finding.resource} in ${finding.filePath}` +
  ...
  // ---- auditor framing ----
  let whyAnAuditorWouldCare: string;
  if (!control) {
    ...
    if (finding.mapping.confidence === "low") {
      ...
      caveats.push(
        "The control mapping is low confidence. The finding is real; the criterion it is attached to may not be.",
      );
    }
  ...
  // ---- what to fix ----
  const whatToFix =
    `${finding.blastRadius.actionVerb}: ${finding.blastRadius.explanation} ` +
  ...
  strengths.push(
    "This resource is defined as code, which means the fix is reviewable in a pull request — and that pull request is itself partial CC8.1 change-management evidence.",
  );
  if (finding.mapping.confidence === "high" || finding.mapping.confidence === "medium") {
    strengths.push(
      `The mapping to ${finding.mapping.controlId} is ${finding.mapping.confidence} confidence (${finding.mapping.mappingSource}), so this can be cited with reasonable safety.`,
    );
  }
  if (shape === "one-change" && affectedInThisScan > 1) {
    strengths.push(
      `This is an account-level fix covering ${affectedInThisScan} findings at once — unusually good effort-to-coverage ratio.`,
    );
  }

  if (shape === "per-resource" && affectedInThisScan > 5) {
    weaknesses.push(
      `${affectedInThisScan} resources need this same fix individually. Consider whether a module or a policy-as-code guardrail would prevent recurrence, rather than fixing each one.`,
    );
  }
  if (finding.severity === "UNKNOWN") {

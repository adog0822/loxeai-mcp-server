...
import {
  ...
  coverageSummary,
  getControl,
} from "../catalog/soc2-controls.js";
import { QUESTIONS, buildApplicabilityBrief, type Answers } from "../applicability/questionnaire.js";
...
import { POLICY_CATALOG, draftPolicy, type PolicyKind } from "../policy/draft.js";
...
import { SEVERITY_RANK, type Severity } from "../scanner/types.js";
...
export function registerTools(server: McpServer): void {
    ...
    "scan_iac",
    {
      ...
      outputSchema: {
        ...
        fingerprint: z
          .string()
          .describe(
            "SHA-256 over the scanned input. An identical fingerprint across two scans proves the input did not change.",
          ),
        scanner: z.string(),
        ...
        suppressions: z
          ...
          .describe(
            "Checks a developer explicitly suppressed in source. These do NOT appear as findings; a suppressed check is an accepted risk, not an absent one.",
          ),
    ...
    async ({ path, frameworks, scanner, configPath }) => {
      try {
        ...
        const structured = {
          ...
          caveat:
            "An IaC scan evidences INTENDED configuration. It is not evidence of deployed state. " +
        ...
        const summaryLines = [
          ...
          ...SEVERITY_ORDER.filter((s) => batch.counts.bySeverity[s] > 0).map(
            (s) => `  ${s}: ${batch.counts.bySeverity[s]}`,
          ),
        ...
        if (error instanceof ScannerUnavailableError) return fail(error.message);
        if (error instanceof TrustError) return fail(error.message);
        return fail(
          `Scan failed: ${safeErrorMessage(error, "unknown scanner error")}\n\n` +
  ...
  server.registerTool(
    "list_findings",
    {
      ...
      inputSchema: {
        ...
        controlId: z
          .enum(SUPPORTED_CONTROL_IDS as [string, ...string[]])
          .optional()
    ...
    async ({ batchId, minSeverity, controlId, unmappedOnly, limit, cursor }) => {
      ...
      try {
        ...
        if (error instanceof CursorMismatchError) return fail(error.message);
        throw error;
      }

      const structured = { batchId, ...page };

      const rows = page.findings.map(
        (f) =>
          `${f.severity.padEnd(8)} ${(f.controlId ?? "UNMAPPED").padEnd(9)} ${f.checkId}  ${f.filePath}  ${f.resource}`,
      );
  ...
  server.registerTool(
    "get_finding",
    {
      ...
      description:
        "Returns a single finding with its SOC 2 control mapping (including how the mapping " +
    ...
    async ({ batchId, findingId }) => {
      const batch = getBatch(batchId);
      if (!batch) return fail(`Unknown batchId "${batchId}". Call scan_iac first.`);

      const finding = batch.findings.find((f) => f.id === findingId);
      if (!finding) {
        return fail(
          `No finding "${findingId}" in batch ${batchId}. Use list_findings to get valid finding IDs.`,
        );
      }
  ...
  server.registerTool(
    ...
    async ({ batchId, findings }) => {
      let items: Array<{ controlId: string; title: string; severity: string; priority: number }>;
      ...
      if (batchId) {
        ...
        items = findings.map((f) => ({
          ...
          priority: severityPriority((f.severity ?? "UNKNOWN") as Severity),
        }));
      } else {
      ...
      const text = [
        ...
        "",
        ...structured.groups.map(
          (g) => `[${g.severity}] ${g.scopeLabel} - ${g.actionVerb}: ${g.title} (${g.count} affected, ${g.controlId})`,
        ),
      ].join("\n");
  ...
  server.registerTool(
    "scanner_status",
    {
      ...
      description:
        "Reports which IaC scanners are installed, which control framework is supported, and " +
    ...
    async () => {
      ...
      return ok({
        ...
        frameworkScope:
          `SOC 2 Trust Services Criteria, Security category only — all ${cov.total} Common Criteria are catalogued. ` +
        ...
        trustedRoots: roots,
        networkEgress:
          "This server itself makes no outbound network requests. It does spawn subprocesses that can: " +
  ...
  server.registerTool(
    "applicability_brief",
    {
      ...
      description:
        "Answer eight questions about how your company is set up and get back, for each of the 33 SOC 2 " +
    ...
    async (args) => {
      const brief = buildApplicabilityBrief(args as Answers);
      const text = [
      ...
      return ok(brief as unknown as Record<string, unknown>, text);
    },
  );
  ...
  server.registerTool(
    "applicability_questions",
    {
      ...
      description:
        "Returns the eight questions used by `applicability_brief`, with the allowed values for each and " +
    ...
    async () =>
      ok({ questions: QUESTIONS } as unknown as Record<string, unknown>),
  );
  ...
  server.registerTool(
    "explain_finding",
    {
      ...
      description:
        "Takes a finding from a scan and explains it without jargon: what it actually means, the real-world " +
    ...
    async ({ batchId, findingId }) => {
      ...
      if (!batch) {
        return fail(
          `No scan batch "${batchId}". Batches are held in memory for this server process only. ` +
      ...
      return ok(explanation as unknown as Record<string, unknown>, text);
    },
  );
  ...
  server.registerTool(
    "preview_trust_page",
    {
      ...
      description:
        "Builds a per-criterion evidence summary from a scan, suitable for showing a prospect's security " +
    ...
    async ({ batchId }) => {
      ...
      if (!batch) {
        return fail(
          `No scan batch "${batchId}". Available: ${listBatchIds().join(", ") || "none"}. Run scan_iac first.`,
        );
      }
      ...
      return ok(page as unknown as Record<string, unknown>, text);
    },
  );
  ...
  server.registerTool(
    "render_trust_page",
    {
      ...
      description:
        "Returns the trust page as a complete HTML or Markdown document, as a STRING. This server never " +
  ...
  server.registerTool(
    "check_capabilities",
    {
      ...
      description:
        "Reports which cloud/VCS CLIs are authenticated on this machine and whether those credentials are " +
    ...
    async () => {
      ...
      const lines: string[] = [report.summary.headline, "", report.serverGuarantee, ""];
      for (const c of report.credentials) {
        if (!c.installed) continue;
        lines.push(`${c.tool}`);
        lines.push(`  authenticated: ${c.authenticated}${c.identity ? ` (${c.identity})` : ""}`);
        if (c.scopes.length > 0) lines.push(`  scopes: ${c.scopes.join(", ")}`);
        lines.push(`  write capability: ${c.writeCapability}`);
        lines.push(`  ${c.assessment}`);
        if (c.narrowerAlternative) lines.push(`  NARROWER OPTION: ${c.narrowerAlternative}`);
        lines.push("");
      }
      lines.push("CAVEATS:");
      for (const c of report.caveats) lines.push(`  - ${c}`);
      return ok(report as unknown as Record<string, unknown>, lines.join("\n"));
    },
  );
  ...
  server.registerTool(
    "export_oscal",
    {
      ...
      description:
        "Emits the scan as a NIST OSCAL assessment-results document (JSON), the machine-readable format used " +
    ...
    async ({ batchId }) => {
      ...
      const results = (doc["assessment-results"] as Record<string, unknown>)["results"] as Array<
        Record<string, unknown>
      >;
      const emitted = (results[0]?.["findings"] as unknown[] | undefined)?.length ?? 0;
      return {
  ...
  server.registerTool(
    "draft_policy",
    {
      ...
      description:
        "Produces a policy DRAFT in Markdown, grounded in the answers you gave to applicability_brief and, " +
    ...
    async (args) => {
      const { kind, companyName, batchId, ...rest } = args as {
        kind: PolicyKind;
        companyName?: string;
        batchId?: string;
      } & Record<string, unknown>;
      ...
      const draft = draftPolicy({
        ...
        ...(answered ? { answers: rest as unknown as Answers } : {}),
        ...(batch ? { batch } : {}),
      });
  ...
  server.registerTool(
    "list_policies",
    {
      ...
      description:
        "Returns the policies this server can draft, the SOC 2 criteria each contributes toward, and what " +
    ...
    async () =>
      ok({
        ...
        note:
          "A policy document contributes toward a criterion; it never satisfies one on its own. SOC 2 wants evidence the policy was approved, communicated, acknowledged and followed.",

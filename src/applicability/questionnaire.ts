...
export type ApplicabilityBrief = {
  ...
  startHere: Array<{ id: string; title: string; why: string }>;
...
function baseWhatYouNeed(control: Soc2Control, a: Answers): string {
  switch (control.id) {
    case "CC1.1":
...
function classify(control: Soc2Control, a: Answers): { applicability: Applicability; rationale: string } {
  ...
  if ((control.id === "CC6.4" || control.id === "CC6.5") && a.premises === "fully-remote" && a.cloud !== "on-prem") {
    return {
      ...
      rationale:
        "You have no office and no owned hardware, so datacenter physical security is normally addressed by carving out your cloud provider and citing their SOC 2 report. This is still IN SCOPE — it is satisfied by inheritance, not by omission. Laptop handling remains yours.",
    };
  }
  ...
  if (control.id === "CC6.3" && a.productionAccess === "solo") {
    return {
      ...
      rationale:
        "You are the only person who can change production, so segregation of duties cannot be satisfied by separating people. Auditors accept compensating controls here — logging, alerting on your own actions, an external reviewer — but you must name them explicitly.",
    };
  }
  ...
  if (control.id === "CC6.2" && a.workforce !== "contractors-only" && a.timeline === "type2-window-open") {
    return {
      applicability: "applies-elevated-risk",
      rationale:
        "Your Type II window is open, so every termination from here forward becomes a sampled population. Offboarding is the most common source of first-audit exceptions.",
    };
  }
  ...
  if ((control.id === "CC7.1" || control.id === "CC6.1") && a.iacCoverage === "none") {
    return {
      applicability: "applies-elevated-risk",
      rationale:
        "None of your infrastructure is defined as code, so there is no reviewable record of intended configuration. Evidence has to come from live cloud state instead, and this tool cannot help you here.",
    };
  }
...
function priorityFor(control: Soc2Control, a: Answers, applicability: Applicability): number {
  let score = 40;

  if (applicability === "applies-elevated-risk") score += 35;
  if (applicability === "inherited-via-carve-out") score -= 25;
  ...
  if (populationBearing.includes(control.id)) {
    if (a.timeline === "type2-window-open" || a.timeline === "in-audit") score += 20;
    else if (a.timeline === "type1-soon") score += 10;
  }

  // Documents are cheap to produce late; activities are not.
  if (control.primarySource === "document") score -= 10;
  if (control.primarySource === "human-process") score += 5;

  // If the tool can act on it now, surface it — it is the cheapest progress.
  if (control.iac === "primary" && a.iacCoverage !== "none") score += 10;

  return Math.max(0, Math.min(100, score));
}

function scopeNotes(a: Answers): string[] {
  const notes: string[] = [];

  if (a.customerData === "personal-data" || a.customerData === "regulated-data") {
    notes.push(
      "You handle personal data. Consider whether the Confidentiality (C1.1-C1.2) or Privacy (P1-P8, 18 criteria) categories should be in scope alongside Security. They are ADDITIVE to these 33, not a substitute.",
    );
  }
  if (a.customerData === "regulated-data") {
    notes.push(
      "Regulated data often brings a second framework entirely (HIPAA, PCI DSS, or similar). SOC 2 does not satisfy those. This tool covers SOC 2 Security only.",
    );
  }
  if (a.cloud === "multi-cloud") {
    notes.push(
      "Multi-cloud means each provider is a separate subservice organization to carve out, and each needs its own configuration evidence.",
    );
  }
  if (a.timeline === "type2-window-open" || a.timeline === "in-audit") {
    notes.push(
      "Type II needs a COMPLETE population across the window, not a point-in-time snapshot. Start capturing offboarding records, access reviews and change approvals now — these cannot be reconstructed after the fact.",
    );
  }
  if (a.timeline === "exploring") {
    notes.push(
      "If you are exploring: SOC 2 is usually driven by a specific enterprise deal. If no customer has asked, the honest answer may be that you do not need it yet.",
    );
  }
  if (a.iacCoverage === "none") {
    notes.push(
      "With no infrastructure-as-code, this tool's scanner has nothing to read. The catalog and this brief are still useful; the scanning is not.",
    );
  }
  return notes;
}
...
export function buildApplicabilityBrief(answers: Answers): ApplicabilityBrief {
  const controls: ControlBrief[] = SOC2_CONTROLS.map((control) => {
    ...
    return {
      ...
      thisToolHelps: control.iac === "none" ? ("no" as const) : control.iac,
